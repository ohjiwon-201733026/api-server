...
public interface FeedRepository extends JpaRepository<Feed, Long> {
    ...
    @Query("select f from Feed f where f not in (select r.feedId from Report r where r.userId = :userId) and f.status = :status and f.category = :category order by f.createdAt desc")
    ...
    @Query("select f from Feed f where f not in (select r.feedId from Report r where r.userId = :userId) and f.status = :status and f.category = :category order by f.likeCount desc")
